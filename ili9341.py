...
def init(sensors):
    ...
    tft = ILI9341(spi,cs=Pin(32),dc=Pin(33),rst=None,rotation=90,width=WIDTH,height=HEIGHT)
    tft.draw_rectangle(BUFFER//2,BUFFER//2,WIDTH-2*(BUFFER//2),HEIGHT-2*(BUFFER//2),fcolor565(1.0,0.5,0.0))
    def title(txt):
        tft.draw_text(BUFFER, BUFFER, txt, fcolor565(1.0,1.0,1.0), background=0)
    def status(txt):
        tft.draw_text(BUFFER, HEIGHT-BUFFER-LINEHEIGHT, txt, fcolor565(0.0,1.0,0.0), background=0)
    def body(txt):
        for i,line in enumerate(txt.split('\n')):
            tft.draw_text(BUFFER, BUFFER+(i+1)*LINEHEIGHT, line, fcolor565(0.0,1.0,1.0), background=0)
            if i == 22:
                break
            
    
    boot_status = str(sensors)
    print(boot_status)
    
    title('Hive Monitor v0')
...
class ILI9341(object):
    ...
    def draw_text(self, x, y, text, color,  background=0,
                     rotate=0):
    ...
    def reset_mpy(self):
        assert self.rst is not None, 'No reset pin configured'
        self.rst(0)
        sleep(.05)
        self.rst(1)
        sleep(.05)
